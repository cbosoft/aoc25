...
local function dfs(state, history, goal, actions)
  local next_history = history + 1
  local rv = nil
  for _, action in ipairs(actions) do
    local next_state = apply_action(action, state)
    local check_result = check(next_state, goal)

    if check_result == 0 then
      return { history=next_history, state=next_state }
    elseif check_result < 1 then
      local res = dfs(next_state, next_history, goal, actions)
      if res ~= nil then
        if rv == nil then
          rv = res
        elseif res.history < rv.history then
          rv = res
        end
      end
    end
...
local function main()
  ...
  for _, i in ipairs(instructions) do
    local res = dfs(i.initial_state, 0, i.goal_state, i.buttons)
    sum = sum + res.history
